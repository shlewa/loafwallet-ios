...
class DynamicDonationViewController: UIViewController, Subscriber {
    ...
    private func updateDonationLabels(donationAmount: Satoshis) {
        ...
        guard let reduxState = store?.reduxState else {
            NSLog("ERROR: State not initialized")
            return
        }
        
        self.finalDonationAmount = donationAmount
        sendAmountLabel.text = DisplayAmount(amount: donationAmount, reduxState: reduxState, selectedRate: reduxState.currentRate, minimumFractionDigits: 2).combinedDescription
        let feeAmount = sender.feeForTx(amount: donationAmount.rawValue)
        networkFeeLabel.text = DisplayAmount(amount:Satoshis(rawValue: feeAmount), reduxState: reduxState, selectedRate: reduxState.currentRate, minimumFractionDigits: 2).combinedDescription.combinedFeeReplacingZeroFeeWithOneCent()
        totalCostLabel.text = DisplayAmount(amount: donationAmount + Satoshis(rawValue: feeAmount), reduxState: reduxState, selectedRate: reduxState.currentRate, minimumFractionDigits: 2).combinedDescription
